# =============================================================================
# STAGE 1: Rotated FCOS + DINOv3 ViT-L/16 + ViT-Adapter — FROZEN ViT (DIOR-R)
# =============================================================================
# Freeze the pretrained DINOv3 ViT; train only the adapter (SPM + deformable
# interactions + projection), FPN and FCOS head. This adapts the strong frozen
...
# (rotated_fcos_dinov3_vitl_adapter_stage2_trainval_dior.py) which loads this
# checkpoint, unfreezes the ViT, and fine-tunes end-to-end at a low backbone LR.
#
# Usage:
#   bash scripts/fcos_vitl_adapter_trainval.sh          # runs stage1 then stage2
#   # or stage 1 only:
#   CUDA_VISIBLE_DEVICES=0,1,2,3 NUM_GPUS=4 STAGE=1 bash scripts/fcos_vitl_adapter_trainval.sh
# =============================================================================
...
# ----------------------------- Optimization --------------------------------
# Backbone (ViT) is frozen -> only adapter + FPN + FCOS head are trained.
# lr is calibrated for the ViT-L recipe used by fcos_vitl_adapter_trainval.sh:
# effective batch 128 (8 GPUs x samples_per_gpu=16, the script default). The
# detection stack (adapter + FPN + FCOS) trains at 4e-4, matching the proven-good
# Oriented R-CNN ViT-Adapter recipe on DIOR-R. If you change GPU count or
# samples_per_gpu, re-scale lr ~linearly (e.g. batch 64 -> lr ~2e-4).

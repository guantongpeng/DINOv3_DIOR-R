...
#
# Usage:
#   bash tools/dist_train_adapter_twostage.sh                      # stage1 -> stage2
#   STAGE=1 bash tools/dist_train_adapter_twostage.sh              # stage 1 only
#   STAGE=2 STAGE1_CKPT=work_dirs/.../best_mAP_epoch_30.pth \
#       bash tools/dist_train_adapter_twostage.sh                  # stage 2 only
#
# Common overrides (environment variables):
#   CUDA_VISIBLE_DEVICES=0,1,2,3   # GPUs to use
#   NUM_GPUS=4                     # must match the list above
#   SAMPLES_PER_GPU=4              # batch size per GPU (adapter is memory-heavy)
...
if [ "${NGPU_LIST}" -ne "${NUM_GPUS}" ]; then
    echo "WARNING: CUDA_VISIBLE_DEVICES lists ${NGPU_LIST} GPUs but NUM_GPUS=${NUM_GPUS}; using ${NGPU_LIST}."
    NUM_GPUS=${NGPU_LIST}
fi
...
if [ "${STAGE}" = "all" ] || [ "${STAGE}" = "2" ]; then
    # Resolve the stage-1 checkpoint to load into stage 2.
    if [ -z "${STAGE1_CKPT}" ]; then
        # Prefer the best-by-mAP checkpoint produced by save_best.
        STAGE1_CKPT=$(ls -t "${S1_DIR}"/best_mAP_epoch_*.pth 2>/dev/null | head -1 || true)
        if [ -z "${STAGE1_CKPT}" ]; then
...
echo "  WORK_DIR=${S2_DIR} SAVE_VIS=0 NUM_GPUS=${NUM_GPUS} bash tools/test.sh"
